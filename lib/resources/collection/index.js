...
Collection.prototype.save = function (ctx, fn) {
  ...
  function done(err, item) {
    ...
    if (domain && domain.hiddenKeys && domain.hiddenKeys.length) {
      domain.hiddenKeys.forEach(function(key) {
        if (item[key]) {
          debug("removing hidden key", key);
  ...
  domain.hiddenKeys = [];
  // overwrites hide method so property still exists when item is written to DB.
  // it will be deleted later before it's returned to the user
  domain.hide = function(property) {
    if (domain.data.hasOwnProperty(property)) {
      domain.hiddenKeys.push(property);
    }
  };
