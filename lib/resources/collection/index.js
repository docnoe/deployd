...
function setUniqueIndex(store, field) {
  store.getCollection(function(err, c) {
    c.ensureIndex(field, {unique: true}, function(err, res) {
      debug("Could not create index %j", err);
      if(res){
        debug("Created index", res);
      }
    });
  });
}

function removeUniqueIndex(store, field) {
  store.getCollection(function(err, c) {
    c.dropIndex(field + "_1", function(err, res) {
      debug("Could not drop index %j", err);
      if(res){
        debug("Dropped index %j", res);
      }
    });
  });
}
  ...
  if(properties && typeof properties === "object"){
    Object.keys(properties).forEach(function(key) {
      if (properties[key].unique) {
        setUniqueIndex(store, key);
      } else {
        removeUniqueIndex(store, key);
      }
    });
  }
