...
(function() {
  ...
  vm.searchQuery.subscribe(function(val) {
    if(val){
      loadPage();
    }
  })
  function loadPage(page, fn) {
    ...
    var query = {$skip: begin*PAGE_SIZE, $limit: PAGE_SIZE*3, $skipEvents: true};
    var searchQuery = vm.searchQuery() //|| '{"foo":"hi"}';
    ...
    dpd(resource).get(query, function(res, err) {
      if (err) return;
      vm.currentPage(page);
